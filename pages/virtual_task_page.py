...
if response:
    ...
    with st.expander("Neues virtuelles Register anlegen"):
        ...
        amplitude = st.number_input("Amplitude", value=1,min_value=1,max_value=1000)
        noise_level = st.number_input("noise_level", value= 1,min_value=1,max_value=20)
    ...
    with st.expander("Neues Task mit virtuellen Registern auf dem Client anlegen"):
        ...
        if response_callables and response_callables.status_code == 200:
            ...
            if click and name and func and interval:
                virtual_task = VirtualTask(name=name, interval=interval, func=func, virtual_register=virtual_registers
                                         )
                print(virtual_task.dict())
                try:
